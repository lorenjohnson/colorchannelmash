...
def adjust_contrast(image, target_contrast, color_space='BGR'):
    ...
    if current_contrast == 0:
        ...
        # Correct for potential inversion
        if contrast_ratio > 1:
            adjusted_image = cv2.convertScaleAbs(image, alpha=contrast_ratio, beta=int(max_pixel_value * (1 - contrast_ratio)))
        else:
            adjusted_image = cv2.convertScaleAbs(image, alpha=1, beta=int(max_pixel_value * (1 - contrast_ratio)))
